...
void periodCalc(int pa)
{
	...
	for(t=1;t<=MAXPERO;t++){
		for(p=0;p<nxcell;p++){
			...
			afterDerr[p] = afterDerr[p]*abs(derr(na,afterPos[p]));
		}
		for(p=0;p<nxcell-1;p++)
			if((afterPos[p]>origPos[p]) &&
			   (afterPos[p+1]<origPos[p+1])&&
			   afterDerr[p]<=1.000){
			   //if(t<35) continue;
			   if(!nPeriod[p]) nPeriod[p] = t;
			   //if(abs(afterPos[p+1]-afterPos[p])>abs(origPos[p+1]-origPos[p]))//Statibility test
			   	//nPeriod[p] = 0;
			}//else if(nPeriod[p]&&(t%nPeriod[p])==0) nPeriod[p]=-1;
	}
